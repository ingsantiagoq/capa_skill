...
// `capa route ADR-XXXX --objetivo <slug> [--route a,b]` — lee o redefine la ruta de un CAPA que
// ya existe. Sin esto la ruta sólo era escribible al nacer: un objetivo creado en blanco quedaba
// bloqueado para siempre, porque el edit-guard exige `route` y `capa new` se niega a repetirse.
function setRoute({ root, config, adr, objetivo, route }) {
  if (!adr || !/^ADR-\d{3,4}$/i.test(adr)) die('uso: capa route ADR-XXXX --objetivo <slug> [--route a,b]');
  if (!objetivo) die('falta --objetivo <slug>');
  adr = adr.toUpperCase();
  const visionDir = adrFolder(root, config, adr);
  if (!visionDir) die(`no existe la visión de ${adr}.`);
  const objSlug = slugify(objetivo);
  const manifestPath = path.join(visionDir, objSlug, 'manifest.json');
  if (!fs.existsSync(manifestPath)) die(`no existe el CAPA ${adr}/${objSlug}. Creálo: capa new ${adr} --objetivo ${objSlug} --route <paths>`);

  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  if (route === undefined) {
    const current = Array.isArray(manifest.route) ? manifest.route : [];
    console.log(current.length ? current.join('\n') : c.dim('(sin ruta declarada)'));
    return;
  }

  const routeArr = String(route).split(',').map((s) => s.trim()).filter(Boolean);
  if (!routeArr.length) die('--route no puede quedar vacía; la ruta es lo que el edit-guard consulta.');
  manifest.route = routeArr;
  fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2) + '\n');
  console.log(c.green('✓ ') + `ruta de ${adr}/${objSlug}: ` + routeArr.join(', '));
}

module.exports = { init, vision, newCapa, setRoute, render, slugify };
