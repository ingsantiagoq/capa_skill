...
const { c, die, readJSON, hasScopeProof } = require('./util');
...
function blockedReason(m) {
  const reasons = [];
  const hasProof = hasScopeProof(m);
...
function buildDb(root, config) {
  ...
  for (const dir of findCapas(capaDir)) {
    ...
    const hasProof = hasScopeProof(m) ? 1 : 0;
    ...
    ins.run(
      ...
      hasProof, pending, blockedReason(m).join(' · '), (m.route || []).join(', '), m.status?.barrido || ''
    );
  }
...
function renderHtml(db, meta) {
  ...
  const bcount = (rows) => rows.reduce((a, x) => { const k = x.barrido || 'SD'; a[k] = (a[k] || 0) + 1; return a; }, {});
