...
function renderHtml(db, meta) {
  ...
  const idxRow = (v) => {
    ...
    // lifecycle muy por debajo de lo construido = campo sin mantener, no trabajo faltante
    const stale = t && iE - life >= Math.max(3, t * 0.2) ? ' stale' : '';
    ...
    return `<tr>
      ...
      <td class="n ax2${stale}" title="lifecycle: flag de workflow, sin prueba detrás${stale ? ' — desactualizado respecto de lo construido' : ''}">${life}<small>/${t}</small></td>
    </tr>`;
  };
