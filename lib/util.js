...
// Un comando es "solo-presencia" si TODAS sus operaciones se limitan a constatar que un
// artefacto EXISTE / PARSEA / VALIDA, sin ejecutar nada que pueda fallar ante un cambio de
// comportamiento: `test -f x` pasa mientras el archivo exista, `bash -n s.sh` sólo chequea
// sintaxis, `docker compose config` sólo valida el yaml. Ninguno es prueba de Alcance —
// son "el andamio existe", no "el comportamiento corre" — así que no cuentan como prueba de
// infra (DP-12).
//
// LÍMITE CONOCIDO Y DELIBERADO: esto sólo ve el COMANDO, no lo que el test asserta por dentro.
// Un `dotnet test` cuyo cuerpo sólo hace File.Exists (p.ej. Stryker_config_file_exists) PASA
// este filtro porque el comando ejecuta un test real. Esa clase de teatro se caza leyendo el
// fuente del test (lint de comportamiento del lado del repo), no estáticamente acá. Este
// helper sólo APRIETA: nunca convierte en válido algo que antes no lo era.
//
// Conservador a propósito: `grep`/comandos desconocidos NO son solo-presencia (podrían ser un
// source-scan real que muerde), para no bloquear gates legítimos por falso positivo.
const PRESENCE_SEG = /^(test\s+-[fxeds]\b|\[\s+-[fxeds]\b|(bash|sh)\s+-n\b|ls(\s|$)|echo\b|printf\b|cat\s|true\s*$|:\s*$|docker\s+compose\b.*\bconfig\b)/i;
function isPresenceOnlyCommand(cmd) {
  const s = String(cmd == null ? '' : cmd).split('#')[0].trim();
  if (!s) return false;
  const segs = s.split(/&&|\|\||;|\n/).map((x) => x.trim()).filter(Boolean);
  if (!segs.length) return false;
  return segs.every((seg) => PRESENCE_SEG.test(seg));
}
...
function hasScopeProof(m, gov) {
  ...
  if (
    ...
    evidence.some((e) => e && INFRA_PROOF_KINDS.includes(e.kind) && e.command && String(e.command).trim() && !isPresenceOnlyCommand(e.command))
  ) {
    return true;
  }
  return false;
...
// A native addon (better-sqlite3) is loaded via the CLI's module graph, and on
// Node >=24 calling process.exit() while it is loaded segfaults intermittently
// during teardown. So we never call process.exit(): we set process.exitCode and
// let the process drain naturally. die() aborts its caller by throwing a
// sentinel that the CLI entrypoints catch (see bin/capa.js).
class CapaExit extends Error {
  constructor(code = 1) { super('__capa_exit'); this.__capaExit = true; this.code = code; }
}

function die(msg) {
  console.error(c.red('✗ ') + msg);
  process.exitCode = 1;
  throw new CapaExit(1);
}
...
module.exports = { c, CapaExit, DIMENSIONS, DECISION, IMPLEMENTATION, LIFECYCLE, EVIDENCE_KINDS, PROOF_KINDS, INFRA_PROOF_KINDS, INFRA_PROOF_GRANT, FRONT_DESIGN_SKILLS, isInfra, hasScopeProof, isPresenceOnlyCommand, governanceGrantsInfraProof, decisionState, isPendingDecision, adrDirOf, loadGovernance, die, readJSON, writeFileSafe, findConfig, loadConfig };
