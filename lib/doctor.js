...
// comporta como antes.
function routeHasNodes(graph, root, entry) {
  const forms = root ? routeCandidates(root, entry) : [entry];
...
function lintCapa(dir, graph, gov = null, root = null) {
  const findings = [];
  ...
  for (const pfx of route) {
    if (!routeHasNodes(graph, root, pfx)) blk('E8', `route sin nodos en el grafo (stale): ${pfx}`);
  }
...
function runDoctor({ root, config, onlyAdr }) {
  ...
  for (const { dir, rel } of capas.sort((a, b) => a.rel.localeCompare(b.rel))) {
    ...
    const findings = lintCapa(dir, graph, govCache.get(adrKey), root);
