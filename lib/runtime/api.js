    ...
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/evidence') {
        ...
        const out = evidence.add({
          root,
          claim,
          classification: body.classification || body.class || 'UNKNOWN',
          sourceType: body.type || body.sourceType || null,
          filePath: body.file || body.filePath || null,
          symbol: body.symbol || null,
          command: body.command || null,
          resultSummary: body.result || body.summary || body.resultSummary || null,
      ...
      if (req.method === 'POST' && url.pathname === '/tests') {
        ...
        const out = tests.add({
          root,
          testType: body.type || body.testType || null,
          command: body.command || null,
      ...
      if (req.method === 'POST' && url.pathname === '/reviews') {
        const body = await readJson(req);
        const out = reviews.add({
          root,
          status: body.status || 'ok',
          diffSummary: body.summary || body.diffSummary || null,
          findings: body.findings || null,
          riskLevel: body.risk || body.riskLevel || null,
        });
        return json(res, out.ok ? 201 : 409, out);
      }
      if (req.method === 'POST' && url.pathname === '/findings') {
        ...
        const out = findings.add({
          ...
          belongs: body.belongs === undefined ? !Boolean(body.outside) : Boolean(body.belongs),
          action: body.action || 'record',
        });
        return json(res, out.ok ? 201 : 409, out);
      }
