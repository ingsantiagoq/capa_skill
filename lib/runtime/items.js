...
function isStateComplete(db, itemId, state) {
  const row = db.prepare(`
    SELECT id FROM capa_progress
    WHERE item_id = ? AND state = ? AND status = 'ok' AND finished_at IS NOT NULL
    LIMIT 1
  `).get(itemId, state);
  return Boolean(row);
}
...
function moveNext({ root }) {
  ...
  if (item.current_state !== 'NEW' && !isStateComplete(db, item.id, item.current_state)) {
    return { ok: false, item, message: `Current state ${item.current_state} must be completed before moving to ${item.next_state}` };
  }
...
function complete({ root, status = 'ok', summary = 'transition completed' }) {
  ...
  const isReviewRework = item.current_state === 'CODE_REVIEW' && status === 'fail';
  if (isReviewRework) {
    const rework = db.transaction(() => {
      const current = db.prepare(`
        SELECT * FROM capa_items
        WHERE id = ? AND status = 'in_progress' AND current_state = 'CODE_REVIEW'
      `).get(item.id);
      if (!current) {
        const blockers = ['CODE_REVIEW state changed before rework completed'];
        return { ok: false, item, status, blockers, message: `Cannot complete CODE_REVIEW: ${blockers.join('; ')}` };
      }

      const failedReviews = db.prepare(`
        SELECT COUNT(*) AS count FROM capa_code_reviews
        WHERE item_id = ? AND status = 'fail'
      `).get(item.id).count;
      if (failedReviews < 1) {
        const blockers = ['CODE_REVIEW rework requires at least one fail review'];
        return { ok: false, item, status, blockers, message: `Cannot complete CODE_REVIEW: ${blockers.join('; ')}` };
      }
      ...
      db.prepare(`
        ...
        VALUES (?, 'CODE_REVIEW', 'fail', ?, ?, ?)
      `).run(item.id, summary, ts, ts);
      db.prepare("UPDATE capa_code_reviews SET status='superseded' WHERE item_id=? AND status='ok'").run(item.id);
      db.prepare("UPDATE capa_progress SET status='superseded' WHERE item_id=? AND state='CODE_REVIEW' AND status='ok'").run(item.id);
      db.prepare("UPDATE capa_code_reviews SET status='reworked' WHERE item_id=? AND status='fail'").run(item.id);
      db.prepare("UPDATE capa_progress SET status='reworked' WHERE item_id=? AND state='CODE_REVIEW' AND status='fail'").run(item.id);
      const updated = db.prepare(`
        UPDATE capa_items SET status='in_progress', current_state='IMPLEMENT', next_state='BUILD', updated_at=?
        WHERE id=? AND status='in_progress' AND current_state='CODE_REVIEW'
      `).run(ts, item.id);
      if (updated.changes !== 1) throw new Error('CODE_REVIEW rework lost state race');
      return { ok: true, item: { ...item, next_state: 'IMPLEMENT' }, status };
    });
    return rework.immediate();
  }

  const blockers = exitCriteria.blockers(db, item, status);
  if (blockers.length) {
    return { ok: false, item, status, blockers, message: `Cannot complete ${item.current_state}: ${blockers.join('; ')}` };
  }
