...
function isAllowedByScope(db, itemId, filePath) {
  ...
  const allowed = rows.some((r) => normalized === normalizePath(r.allowed_path) || normalized.startsWith(`${normalizePath(r.allowed_path)}/`));
...
function evaluate({ root, action, file = null, autoFix = false }) {
  ...
  if (item.status === 'blocked') return block(`PBI is blocked: #${item.id} ${item.title}`);
